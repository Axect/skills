...
column / array names. The rest of the script (style context, `pparam`,
...
- All four loaders coexist with `import matplotlib.pyplot as plt` and
  `import scienceplots` from the plot template. Keep `scienceplots` even
  though linters flag it as unused — it registers the style by import
  side-effect.
